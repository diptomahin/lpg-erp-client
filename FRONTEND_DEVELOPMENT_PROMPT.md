...
- The business buys LPG in TON and stores inventory in KG.
...
```text
quantityKg = quantityTon × 1000
subtotal = quantityTon × purchaseRatePerTon
totalCost = subtotal + additionalCost
```

The backend calculates and stores final values. Display purchase number, quantity TON/KG, rate, additional cost, total cost, paid, due, payment status, batch number, remaining KG, batch cost/KG, and batch status.
