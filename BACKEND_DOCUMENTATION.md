...
Sale input contains cylinder type, cylinder count, and price per cylinder. The backend derives the historical rate per KG from the cylinder capacity and calculates:

```text
item LPG = cylinder capacity × cylinder count
total LPG = sum of item LPG
item amount = cylinder count × price per cylinder
total amount = sum(item amount) - discount
```
...
For each sale item, the server derives `ratePerKg` as `pricePerCylinder / capacityKg` and stores both values. This preserves the entered cylinder price while providing the per-KG rate required for batch costing and profit calculations.
...
The server derives and persists `ratePerKg` as `pricePerCylinder / capacityKg` on each historical sale item. Do not send `ratePerKg`, `totalLpgKg`, `totalAmount`, `totalCost`, `grossProfit`, or `totalDue`; these are calculated and persisted by the server.
...
### Salary and partner distributions

Salary is tracked separately from general operating expenses. The Expenses page must not be used for salary payments.
...
A person may be an employee, a partner, or both. Salary payments require an employee. Profit-share payments require a partner. Salary is included separately in reports and reduces operating profit; partner profit shares are distributions and do not reduce operating profit or gross profit.
...
```json
{
  ...
  "monthlySalary": 32000,
  "notes": "Full-time operations employee"
}
```

`monthlySalary` is the fixed monthly compensation assigned to an employee. This value is stored on the person record and should be shown separately from the actual amount paid in a given month from `/salary-payments`.
...
Daily and monthly reports accept `from` and `to`. Sales and purchase reports also return paginated rows. Sales report rows include `purchasePricePerKg`, `sellingPricePerKg`, and `theoreticalProfit`; theoretical profit is based on sale revenue minus batch COGS and is not reduced by customer payments received.
